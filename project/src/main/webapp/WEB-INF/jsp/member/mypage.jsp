                ...
                <div class="bbs">
                    ...
                    <p><span><strong>총 ${requestScope.result.totalCount }개</strong>  |  ${noticeVO.page }/${result.totalPage }페이지</span></p>
                        ...
                        <c:forEach var="vo" items="${result.list }">        
                            <tr>
                                <td>${vo.no }</td>
                                <td class="txt_l">
                                    <a href="/test/board/notice/view.do?no=${vo.no}">${vo.title }</a>
                                </td>
                                <td class="writer">
                                    ${vo.membername }
                                </td>
                    ...
                    <div class="pagenate clear">
                        ...
                        <c:if test="${result.prev == true }">
                        	<li><a href="mypage.do?page=${result.startPage-1 }&stype=${param.stype }&sword=${param.sword}"> < </a></li>
                        </c:if>
                        <c:forEach begin="${result.startPage }" end="${result.endPage }" var="num">
                            <li><a href='mypage.do?page=${num }&stype=${param.stype }&sword=${param.sword}' <c:if test="${noticeVO.page == num }">class='current'</c:if>>${num }</a></li>
                        </c:forEach>
                        <c:if test="${result.next == true }">
                        	<li><a href="mypage.do?page=${result.endPage+1 }&stype=${param.stype }&sword=${param.sword}"> > </a></li>
                        </c:if>
                        </ul> 
                    </div>
